...
Ulmen text/binary must be smaller than JSON, CSV, TOML, and pickle
...
class TestSizeVsJson:
    def test_py_text_lt_json_1k(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_text()) < json_size(recs_1k)

    def test_rust_text_lt_json_1k(self, recs_1k):
        assert len(UlmenDictRust(recs_1k).encode_text()) < json_size(recs_1k)

    def test_py_binary_lt_json_1k(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_binary_pooled()) < json_size(recs_1k)

    def test_rust_binary_lt_json_1k(self, recs_1k):
        assert len(UlmenDictRust(recs_1k).encode_binary_pooled()) < json_size(recs_1k)

    def test_py_text_lt_json_small(self, recs_small):
        assert len(UlmenDict(recs_small).encode_text()) < json_size(recs_small)

    def test_rust_text_lt_json_small(self, recs_small):
        assert len(UlmenDictRust(recs_small).encode_text()) < json_size(recs_small)

    def test_zlib_lt_json_1k(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_binary_zlib()) < json_size(recs_1k)
...
class TestSizeVsCsv:
    def test_py_text_lt_csv(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_text()) < csv_size(recs_1k)

    def test_rust_text_lt_csv(self, recs_1k):
        assert len(UlmenDictRust(recs_1k).encode_text()) < csv_size(recs_1k)

    def test_py_binary_lt_csv(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_binary_pooled()) < csv_size(recs_1k)

    def test_rust_binary_lt_csv(self, recs_1k):
        assert len(UlmenDictRust(recs_1k).encode_binary_pooled()) < csv_size(recs_1k)
...
class TestSizeVsToml:
    def test_py_text_lt_toml(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_text()) < toml_size(recs_1k)

    def test_rust_text_lt_toml(self, recs_1k):
        assert len(UlmenDictRust(recs_1k).encode_text()) < toml_size(recs_1k)

    def test_py_binary_lt_toml(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_binary_pooled()) < toml_size(recs_1k)
...
class TestSizeVsPickle:
    def test_py_binary_lt_pickle(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_binary_pooled()) < pickle_size(recs_1k)

    def test_rust_binary_lt_pickle(self, recs_1k):
        assert len(UlmenDictRust(recs_1k).encode_binary_pooled()) < pickle_size(recs_1k)

    def test_zlib_lt_pickle(self, recs_1k):
        assert len(UlmenDict(recs_1k).encode_binary_zlib()) < pickle_size(recs_1k)
...
class TestInternalSizeOrdering:
    ...
    def test_full_pool_binary_le_default(self, recs_1k):
        full = UlmenDictFull(recs_1k).encode_binary()
        default = UlmenDict(recs_1k).encode_binary_pooled()
