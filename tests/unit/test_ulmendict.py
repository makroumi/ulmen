"""
Unit tests for the UlmenDict and UlmenDictFull public API.
"""
import json
...
# ===========================================================================
# UlmenDict construction
# ===========================================================================

class TestUlmenDictConstruction:
    ...
    def test_generator_input(self):
        ld = UlmenDict({'x': i} for i in range(3))
        assert len(ld) == 3

    def test_version_attr(self):
        from ulmen.core import __version__
        assert __version__ == UlmenDict.VERSION
    ...
    def test_optimizations_true(self):
        ld = UlmenDict([{'a': 1}], optimizations=True)
    ...
    def test_repr(self):
        ld = UlmenDict([{'a': 1}])
        r = repr(ld)
        assert 'UlmenDict' in r
        assert '1' in r

    def test_slots_present(self):
        assert hasattr(UlmenDict, '__slots__')
        assert '_data' in UlmenDict.__slots__
...
class TestUlmenDictEncoding:
    ...
    def test_encode_binary_optimizations_false(self):
        ld = UlmenDict([{'id': i} for i in range(10)], optimizations=False)
        b = ld.encode_binary()
        assert b[:4] == MAGIC

    def test_encode_binary_optimizations_true(self):
        ld = UlmenDict([{'id': i} for i in range(10)], optimizations=True)
        b = ld.encode_binary()
        assert b[:4] == MAGIC
...
class TestUlmenDictDecoding:
    def test_decode_text_returns_ulmendict(self):
        ld = UlmenDict([{'a': 1}])
        text = ld.encode_text()
    ...
    def test_decode_binary_returns_ulmendict(self):
        ld = UlmenDict([{'a': 1}])
...
class TestUlmenDictToJson:
    ...
    def test_nested_nan(self):
        ld = UlmenDict([{'nested': {'val': float('nan')}}])
        parsed = json.loads(ld.to_json())
        assert parsed[0]['nested']['val'] is None

    def test_list_with_nan(self):
        ld = UlmenDict([{'vals': [1.0, float('nan'), 3.0]}])
        parsed = json.loads(ld.to_json())
...
class TestUlmenDictFull:
    def test_construction(self):
        ldf = UlmenDictFull([{'a': i} for i in range(10)])
        assert len(ldf) == 10

    def test_optimizations_true(self):
        ldf = UlmenDictFull([{'a': 1}])
    ...
    def test_pool_size_limit_256(self):
        ldf = UlmenDictFull([{'k': f'v_{i}'} for i in range(300)])
        assert len(ldf._pool) <= 256


# ===========================================================================
# UlmenDictRust — interface parity (Python shim or real Rust, same contract)
# ===========================================================================
...
class TestUlmenDictRust:
    ...
    def test_encode_binary_zlib(self):
        import zlib
        ld = UlmenDictRust([{'a': i} for i in range(50)])
    ...
    def test_empty_binary(self):
        assert UlmenDictRust([]).encode_binary()[:4] == MAGIC
...
# ===========================================================================
# UlmenDictFullRust — test the interface contract, not internal Python attrs
# ===========================================================================
...
class TestUlmenDictFullRust:
    ...
    def test_construction_empty(self):
        assert len(UlmenDictFullRust([])) == 0
    ...
    def test_pool_size_limit_python_shim_only(self):
        ...
        ldf = UlmenDictFullRust([{'a': 1}], pool_size_limit=128)
        assert ldf._pool_size_limit == 128

    def test_encode_text(self):
        ldf = UlmenDictFullRust([{'id': i} for i in range(5)])
        assert isinstance(ldf.encode_text(), str)
    ...
    def test_encode_binary_zlib(self):
        import zlib
        ldf = UlmenDictFullRust([{'a': i} for i in range(50)])
        assert zlib.decompress(ldf.encode_binary_zlib())

    def test_repr(self):
        ldf = UlmenDictFullRust([{'a': 1}])
...
class TestDecodeBinaryNotList:
    def test_decode_binary_non_list_result(self):
        ...
        with mock.patch("ulmen.core._api.decode_binary_records", return_value={"id": 1}):
            decoded = ld.decode_binary(binary)
            assert len(decoded) == 1


class TestUlmenDictFullNone:
    def test_ulmendictfull_init_none(self):
        # api line 220: UlmenDictFull(None) -> self._data = []
        from ulmen.core._api import UlmenDictFull
        ldf = UlmenDictFull(None)
        assert len(ldf) == 0
        assert ldf._data == []
