...
impl ColVal {
    #[inline(always)]
    fn as_f64(self) -> f64 {
        if let Self::Float(b) = self { f64::from_bits(b) } else { 0.0 }
    }
    #[inline(always)]
...
impl StringTable {
    fn new(cap: usize) -> Self {
        Self { strings: Vec::with_capacity(cap), index: fx_map(cap) }
    }

    #[inline(always)]
    fn intern(&mut self, s: String) -> u32 {
        if let Some(&i) = self.index.get(s.as_str()) { return i; }
    ...
    #[inline(always)]
    fn get(&self, i: u32) -> &str { &self.strings[i as usize] }
...
fn pack_str(b: &[u8], o: &mut Vec<u8>) {
    let l = b.len();
    if l <= 3 { o.push(T_STR_TINY); o.push(l as u8); }
    else       { o.push(T_STR);     varint(l as u64, o); }
    o.extend_from_slice(b);
}

#[inline(always)] fn emit_null(o: &mut Vec<u8>)          { o.push(T_NULL); }
#[inline(always)] fn emit_bool(v: bool, o: &mut Vec<u8>) { o.push(T_BOOL); o.push(v as u8); }
#[inline(always)] fn emit_int(v: i64, o: &mut Vec<u8>)   { o.push(T_INT);  zigzag(v, o); }
#[inline(always)] fn emit_pref(v: u32, o: &mut Vec<u8>)  { o.push(T_POOL_REF); varint(v as u64, o); }
#[inline(always)] fn emit_float(v: f64, o: &mut Vec<u8>) { o.push(T_FLOAT); o.extend_from_slice(&v.to_bits().to_be_bytes()); }
    ...
    match v {
        ...
        ColVal::Str(si)  => {
            if (si as usize) < pi.len() {
                if let Some(p) = pi[si as usize] { emit_pref(p, o); return; }
            }
...
fn fmt_val(v: ColVal, st: &StringTable, pi: &[Option<u32>], o: &mut String) {
    match v {
        ...
        ColVal::Int(n)      => { let _ = write!(o, "{}", n); }
        ColVal::Float(_)    => {
            let f = v.as_f64();
            if f.is_nan()      { o.push_str("nan");  return; }
            if f.is_infinite() { o.push_str(if f > 0.0 { "inf" } else { "-inf" }); return; }
            let _ = write!(o, "{:?}", f);
        }
        ColVal::Str(si) => {
            let s   = st.get(si);
...
fn detect_strat(vals: &[ColVal]) -> u8 {
    ...
    for &v in vals {
        match v {
            ColVal::Null    => { all_bool = false; all_int = false; }
            ColVal::Bool(_) => { all_int  = false; nn += 1; }
            ColVal::Int(_)  => { all_bool = false; nn += 1; }
            _               => { all_bool = false; all_int = false; nn += 1; }
        }
    }
    if nn == 0  { return S_RLE; }
    if all_bool { return S_BITS; }
    if all_int {
        let mut prev = 0i64;
        ...
        return if delta_cost < raw_cost { S_DELTA } else { S_RAW };
    }
    if nn > 4 {
        ...
        'outer: for &v in vals {
            if let ColVal::Str(si) = v {
                for j in 0..u { if small[j] == si { continue 'outer; } }
                if u < 32 { small[u] = si; u += 1; } else { overflow = true; break; }
            }
        }
        let final_u = if overflow {
            ...
            for &v in vals {
                if let ColVal::Str(si) = v {
                    if seen.insert(si, ()).is_none() { u2 += 1; }
                }
            }
            u2
        } else { u };
        if final_u > 0 && final_u <= threshold { return S_POOL; }
    }
    let mut runs = 1usize;
    for i in 1..vals.len() { if !cv_eq(vals[i], vals[i-1]) { runs += 1; } }
    if runs * 10 < vals.len() * 6 { return S_RLE; }
    S_RAW
}
...
fn pack_bits_col(v: &[ColVal], o: &mut Vec<u8>) {
    ...
    for (i, &x) in v.iter().enumerate() {
        if matches!(x, ColVal::Bool(true)) { o[start + (i >> 3)] |= 1 << (i & 7); }
    }
}
...
fn pack_delta_col(v: &[ColVal], o: &mut Vec<u8>) {
    ...
    for &x in v {
        if let ColVal::Int(n) = x {
            zigzag(if first { first = false; n } else { n.wrapping_sub(prev) }, o);
            prev = n;
        }
    }
}
...
fn pack_rle_col(v: &[ColVal], st: &StringTable, pi: &[Option<u32>], o: &mut Vec<u8>) {
    if v.is_empty() { o.push(T_RLE); varint(0, o); return; }
    let mut runs = 1;
    for i in 1..v.len() { if !cv_eq(v[i], v[i-1]) { runs += 1; } }
    o.push(T_RLE);
    varint(runs as u64, o);
    let mut s = 0;
    while s < v.len() {
        let mut e = s + 1;
        while e < v.len() && cv_eq(v[e], v[s]) { e += 1; }
...
fn extract_val(v: &Bound<'_, PyAny>, st: &mut StringTable) -> PyResult<ColVal> {
    ...
    if v.is_instance_of::<PyInt>() {
        if let Ok(n) = v.extract::<i64>() { return Ok(ColVal::Int(n)); }
    }
    if v.is_instance_of::<PyFloat>() {
        if let Ok(f) = v.extract::<f64>() { return Ok(ColVal::from_f64(f)); }
    }
    if v.is_instance_of::<PyString>() {
        let ps = v.downcast::<PyString>()?;
        return match ps.to_str() {
            Ok(s)  => Ok(ColVal::Str(st.intern_str(s))),
            Err(_) => { let s: String = v.extract()?; Ok(ColVal::Str(st.intern(s))) }
        };
    }
    if let Ok(n) = v.extract::<i64>()    { return Ok(ColVal::Int(n)); }
    if let Ok(f) = v.extract::<f64>()    { return Ok(ColVal::from_f64(f)); }
    if let Ok(s) = v.extract::<String>() { return Ok(ColVal::Str(st.intern(s))); }
    Ok(ColVal::Null)
}
...
fn scan_val(v: &Bound<'_, PyAny>, st: &mut StringTable) -> PyResult<()> {
    ...
    if let Ok(l) = v.downcast::<PyList>() {
        for item in l.iter() { scan_val(&item, st)?; }
    }
    Ok(())
}
        ...
        if ok {
            for (ci, (k, v)) in d.iter().enumerate() {
                let keq = if let Ok(ps) = k.downcast::<PyString>() {
                    ...
                    k.extract::<String>().map(|s| s == st.get(keys[ci])).unwrap_or(false)
                };
                if !keq { ok = false; break; }
                buf.push(extract_val(&v, &mut st)?);
            }
        }
        if ok && buf.len() == nc {
            for ci in 0..nc { cd[ci].push(buf[ci]); }
        } else {
...
fn build_pool_inner(
    ...
    for col in cd {
        for &v in col {
            if let ColVal::Str(si) = v {
                if st.get(si).len() > 1 { freq[si as usize] += 1; }
            }
        }
    }
    ...
    for (pos, &si) in pool.iter().enumerate() { pi[si as usize] = Some(pos as u32); }
    (pool, pi)
}
...
fn build_pool_line(p: &[u32], st: &StringTable) -> String {
    ...
    let mut o = String::with_capacity(
        p.iter().map(|&si| st.get(si).len() + 2).sum::<usize>() + 6,
    );
    o.push_str("POOL:");
...
fn build_schema_header(k: &[u32], cd: &[Vec<ColVal>], st: &StringTable) -> String {
    ...
    for (ci, &ki) in k.iter().enumerate() {
        ...
        o.push(':');
        o.push(if cd.is_empty() { 'n' } else { col_type_char(&cd[ci]) });
    }
    o
}
...
fn build_binary_pool_block(p: &[u32], st: &StringTable) -> Vec<u8> {
    ...
    let mut o = Vec::with_capacity(
        1 + 5 + p.iter().map(|&si| st.get(si).len() + 3).sum::<usize>(),
    );
    o.push(T_POOL_DEF);
    varint(p.len() as u64, &mut o);
    for &si in p { pack_str(st.get(si).as_bytes(), &mut o); }
    o
}
...
impl LumenCore {
    fn build(lst: &Bound<'_, PyList>, max_pool: usize, zlib_level: u32) -> PyResult<Self> {
        ...
        let (inline_cols, row_cols) = if nr <= 1 {
            ...
            let ic: Vec<bool> = col_strats.iter()
        ...
        let col_bodies_strat: Vec<Vec<u8>> = cd.iter().enumerate().map(|(ci, v)| {
        ...
        let col_bodies_raw: Vec<Vec<u8>> = cd.iter().map(|v| {
            let mut b = Vec::with_capacity(v.len() * 4 + 8);
        ...
        let row_text_lines: Vec<String> = (0..nr).map(|ri| {
        ...
        let cached_text = Self::assemble_text(
            nr, &pool_line, &schema_header, &matrix_header,
            &inline_cols, &inline_text_blocks, &row_cols, &row_text_lines,
        );
        let cached_binary_strat = Self::assemble_binary(
            nr, nc, &keys, &st, &col_strats, &binary_pool_block,
            &col_bodies_strat, true,
        );
        ...
        Ok(Self {
            ...
            col_types, col_strats, inline_cols, row_cols,
    ...
    fn empty(zlib_level: u32) -> Self {
        ...
        Self {
            ...
            col_types: vec![], col_strats: vec![],
            inline_cols: vec![], row_cols: vec![],
            pool_line: String::new(), schema_header: String::new(),
            matrix_header: String::new(),
            inline_text_blocks: vec![], row_text_lines: vec![],
            binary_pool_block: vec![],
            col_bodies_strat: vec![], col_bodies_raw: vec![],
    ...
    fn assemble_text(
        nr: usize, pl: &str, sh: &str, mh: &str,
        ic: &[bool], itb: &[String], rc: &[usize], rtl: &[String],
    ) -> String {
        if nr == 0 { return String::new(); }
        let mut o = String::with_capacity(
            pl.len() + mh.len() + sh.len()
            + itb.iter().map(|s| s.len() + 1).sum::<usize>()
            + rtl.iter().map(|s| s.len() + 1).sum::<usize>()
            + 16,
        );
        if !pl.is_empty() { o.push_str(pl); o.push('\n'); }
        if nr == 1 {
            o.push_str(sh); o.push('\n');
            if !rtl.is_empty() { o.push_str(&rtl[0]); }
            return o;
        }
        o.push_str(mh); o.push('\n');
        for (ci, blk) in itb.iter().enumerate() {
            if ic[ci] { o.push_str(blk); o.push('\n'); }
        }
        if !rc.is_empty() {
    ...
    fn assemble_binary(
        nr: usize, nc: usize, keys: &[u32], st: &StringTable,
        cs: &[u8], bpb: &[u8], cbs: &[Vec<u8>], use_strat: bool,
    ) -> Vec<u8> {
        let mut o = Vec::with_capacity(
            6 + bpb.len() + 10 + nc * 12
            + cbs.iter().map(|b| b.len()).sum::<usize>(),
        );
        ...
        if nr > 1 {
            ...
            for b in cbs { o.extend_from_slice(b); }
        } else {
            o.push(T_LIST);
            varint(1, &mut o);
            o.push(T_MAP);
            ...
            for (ci, b) in cbs.iter().enumerate() {
                ...
                if b.len() > 2 { o.extend_from_slice(&b[2..]); } else { emit_null(&mut o); }
            }
        }
        o
    }
...
impl LumenDictRust {
    ...
    fn bench_encode_text_clone(&self, iters: usize) -> usize {
        let mut t = 0;
        for _ in 0..iters { t += self.core.text_cached().len(); }
        t
    }
    fn bench_encode_binary_clone(&self, iters: usize) -> usize {
        let mut t = 0;
        for _ in 0..iters { t += self.core.binary_strat_cached().len(); }
        t
    }
    ...
    fn __repr__(&self) -> String {
        format!(
            ...
            self.core.n_rows, self.core.pool.len(), self.opt,
        )
    }
}
...
impl LumenDictFullRust {
    ...
    fn __repr__(&self) -> String {
        format!(
            ...
            self.core.n_rows, self.core.pool.len(), self.lim,
        )
    }
}
...
impl<'a> Cursor<'a> {
    ...
    fn read_str(&mut self) -> PyResult<&'a str> {
        ...
        std::str::from_utf8(bytes).map_err(|e| {
            pyo3::exceptions::PyValueError::new_err(format!("UTF-8 error: {}", e))
        })
    }
    ...
    fn read_tagged_value<'py>(
        ...
        match tag {
            ...
            T_DELTA_RAW => {
                let n    = self.read_varint() as usize;
                let list = PyList::empty_bound(py);
                if n == 0 { return Ok(list.into_any()); }
            ...
            T_RLE => {
                ...
                for _ in 0..n_runs {
                    ...
                    for _ in 0..run { list.append(v.clone().into_any())?; }
                }
                Ok(list.into_any())
            }
            other => Err(pyo3::exceptions::PyValueError::new_err(
                format!("Unknown tag 0x{:02x} at pos {}", other, self.pos - 1)
            )),
        }
    }
}
    ...
    match strat {
        ...
        S_RLE => {
            ...
            for _ in 0..n_runs {
                ...
                for _ in 0..run { out.push(v.clone().into_any()); }
            }
            Ok(out)
        }
        _ => {
            ...
            let n       = cur.read_varint() as usize;
            let mut out = Vec::with_capacity(n);
            for _ in 0..n { out.push(cur.read_value(py, pool)?); }
            Ok(out)
        }
    }
}
...
fn llm_encode_val_into(v: &Bound<'_, PyAny>, out: &mut String) -> PyResult<()> {
    ...
    if let Ok(d) = v.downcast::<PyDict>() {
        if d.is_empty() { out.push_str("{}"); return Ok(()); }
        out.push('{');
        let mut first = true;
        for (k, val) in d.iter() {
...
fn llm_find_top_level(s: &str, ch: char) -> Option<usize> {
    ...
    for (i, c) in s.char_indices() {
        ...
        else if c == ch && depth == 0 { return Some(i); }
    }
    None
}
...
fn llm_decode_nested_dict<'py>(inner: &str, py: Python<'py>) -> PyResult<Bound<'py, PyAny>> {
    let d = PyDict::new_bound(py);
    if inner.is_empty() { return Ok(d.into_any()); }
