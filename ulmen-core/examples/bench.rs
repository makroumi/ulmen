...
fn main() {
    ...
    bench("encode payload", 1000, || {
        let _ = payload.encode();
    });
    bench("decode payload", 1000, || {
        let _ = AgentPayload::decode(&encoded);
    });
    bench("validate payload", 1000, || {
        let _ = validate_payload(&payload);
    });
    bench("validate payload (from string)", 1000, || {
        let _ = validate_payload_str(&encoded);
    });
    ...
    bench("count_tokens", 1000, || {
        let _ = count_tokens(&encoded);
    });
    bench("dedup_mem", 1000, || {
        let _ = dedup_mem(&records);
    });
